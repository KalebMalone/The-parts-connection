      ...
      <Formik
        ...
        onSubmit={async (values) => {
          ...
          try {
            ...
            if (response.ok) {
              // Ensure data.user exists and we get a token
              if (data && data.user && data.user.name) {
                ...
                // Store the user and token if needed (e.g., in localStorage or state)
                localStorage.setItem('user', JSON.stringify(data.user));
                localStorage.setItem('token', data.token);
        
                // If the user is logged in, update context and navigate
                updateUser(data.user);
                navigate("/"); // Redirect to home after successful login/signup
              } else {
                toast.error("User data or token is missing in the response.");
              }
            } else {

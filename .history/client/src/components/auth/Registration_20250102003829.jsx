...
const signupSchema = object({
    ...
    email: string().email("Email must be valid").max(40, "Email must be 40 characters max").required("Email is required"),
    password: string().min(10, "Password must be between 10 and 20 characters long").max(20, "Password must be between 10 and 20 characters long").required("Password is required"),
});

const signinSchema = object({
    email: string().email("Email must be valid").max(40, "Email must be 40 characters max").required("Email is required"),
    password: string().min(8, "Password has to be at least 8 characters long").max(20, "Password must be 20 characters max").required("Password is required"),
});
        ...
        <Container>
            ...
            <Formik
                ...
                onSubmit={async (values) => {
                    ...
                    console.log("Form values being sent:", values); // Add this log
                    try {
                        ...
                        if (response.ok) {
                            if (data.user) {
                                toast.success(isLogin ? `Welcome back, ${data.user.name}!` : `Welcome, ${data.user.name}!`);

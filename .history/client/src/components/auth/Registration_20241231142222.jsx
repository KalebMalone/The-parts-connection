...
useEffect(() => {
    ...
    if (storedUser) {
        try {
            ...
            updateUser(user); // Set user in the context
        } catch (error) {
            ...
            <Formik
                ...
                onSubmit={async (values) => {
                    ...
                    try {
                        ...
                        console.log(data); // Debugging: Inspect the response payload
                        if (response.ok) {
                            const userName = data.user?.name || "User"; // Safely access data.user.name
                            toast.success(
                                isLogin ? `Welcome back, ${userName}!` : `Welcome, ${userName}!`
                            );
                            updateUser(data.user);
                            localStorage.setItem("currentUser", JSON.stringify(data.user));
                            navigate("/"); // Redirect to home after successful login/signup
                        } else {
                            toast.error(data.error || "Something went wrong!"); // Handle error messages
                        }
                    } catch (error) {

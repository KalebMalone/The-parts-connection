...
const Profile = () => {
  ...
  const handleDeleteOrder = async (orderId) => {
    ...
    if (confirmDelete) {
      try {
        // The correct URL should include the orderId
        const response = await fetch(`http://127.0.0.1:5000/api/v1/orders/${orderId}/delete`, {
        ...
        if (response.ok) {
          // Update the local state to remove the deleted order from the list

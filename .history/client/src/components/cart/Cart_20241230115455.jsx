...
const Cart = () => {
  ...
  // Calculate the total price before using it
  ...
  const handleCheckout = async () => {
    ...
    // Adjust order data structure based on the expected format
    const orderData = {
      status: "Pending", // Optional, set based on your app's workflow
      total_price: calculatedTotalPrice, // Total price of the order
      ...
      items: cartItems.map((item) => ({
        ...
        price_per_item: item.price || 0, // Price per item, use 'price' from cart items
        total_price: item.price * item.quantity || 0, // Total price for this item
      })),
    };
